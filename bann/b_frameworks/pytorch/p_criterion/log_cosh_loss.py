...
from typing import Any, final, Final, Tuple

import torch

from bann.b_frameworks.pytorch.interfaces.custom_criterion_interface import \
    PCustomLoss
...
@final
class LogCoshLoss(PCustomLoss[torch.Tensor]):
    ...
    def forward(self, *input_args: Any) -> torch.Tensor:
        ...
        if not(isinstance(results, torch.Tensor) and isinstance(target, torch.Tensor)):
            raise KnownCriterionError(
                f"{LogCoshLoss.__name__} can only work with tensor types as input"
            )
        if target.size() != results.size():
            raise KnownCriterionError(
                f"({LogCoshLoss.__name__}) Mismatch sizes t: {target.size()}; i: {results.size()}"
            )
        dif = results - target
        if self.weight:
            with torch.no_grad():
            ...
            dif *= mul
        # log(cosh(x)) - loss implemented in PyTorch
        ret = torch.log(torch.cosh(dif.double() + 1e-7))
        ################################################
