...
from typing import Dict, Optional, Tuple, Union, Type, Callable, final, Final, List, Any, TypeVar
...
from bann.b_container.states.framework.interface.pytorch.optim_per_parameter import \
    PerParameterStateKwargs, PerParameterAbc
...
@final
@dataclass
class _RMSpropStateKwargs(OptimStateKwargs):
    lr: float = 0.0001
    momentum: float = 0.
    alpha: float = 0.99
    eps: float = 1e-8
...
@final
class RMSpropState(MainOptimSt[_RMSpropStateKwargs], PerParameterAbc):

    def __init__(self) -> None:
        super().__init__()
        self.__kwargs: Optional[_RMSpropStateKwargs] = None
        self.__l_kwargs: Dict[str, _LayerCon] = {}
        self.__l_key_order: List[str] = []

    @staticmethod
    def _order_args() -> List[str]:
        return ['lr', 'momentum', 'alpha', 'eps', 'weight_decay', 'centered']
    ...
    def get_kwargs(self) -> _RMSpropStateKwargs:
    ...
    def _hyper_min_values(self, com_t: Type[_ComVar], l_key: int, /) -> List[_ComVar]:
        erg = []
        ...
        if self.__l_kwargs[self.__l_key_order[l_key]].alpha is not None:
            erg.append(com_t(True, 0))
    ...
    def _max_values(self, l_key: int, /) -> List[CompareNumElem]:
        ...
        if self.__l_kwargs[self.__l_key_order[l_key]].momentum is not None:
            erg.append(CompareNumElem(True, None))
        if self.__l_kwargs[self.__l_key_order[l_key]].alpha is not None:
            erg.append(CompareNumElem(False, 1))
        if self.__l_kwargs[self.__l_key_order[l_key]].weight_decay is not None:
            erg.append(CompareNumElem(True, None))
        return erg
    ...
    def max_values(self) -> Tuple[CompareNumElem, ...]:
        list_erg = [
            ...
            *[
                value_e for l_i in range(len(self.__l_key_order))
                for value_e in self._max_values(l_i)
            ]
        ]
    ...
    def get_hyper_param(self) -> Tuple[float, ...]:
        ...
        return (
            ...
            self.get_kwargs().alpha,
            self.get_kwargs().weight_decay,
            *[value_e for l_i in self.__l_key_order for value_e in layer_dict[l_i].values()]
        )
    ...
    def set_new_hyper_param(self, params: Tuple[float, ...], /) -> None:
        ...
        self.get_kwargs().alpha = compare_min_max_float(
...
_RMSpropStateTypes: Final[Dict[str, Tuple[Callable[[str], object], str]]] = {
    ...
    f'{MainOptimSt.get_pre_arg()}alpha': (
        lambda a_v: get_layer_wise_args(lambda val: float(val) if float(val) >= 0 else 0.0, a_v),
        "float;layer_name:float;... (>=0)"
    ),
    ...
    f'{MainOptimSt.get_pre_arg()}centered': (
...
def get_rmsprop_state_types() -> Dict[str, Tuple[Callable[[str], object], str]]:
    return _RMSpropStateTypes
