...
@final
@dataclass
class _ADAMSparseStateKwargs(OptimStateKwargs):
    lr: float = 0.0001
...
@final
class ADAMSparseState(MainOptimSt[_ADAMSparseStateKwargs], PerParameterAbc):

    def __init__(self) -> None:
        super().__init__()
        self.__kwargs: Optional[_ADAMSparseStateKwargs] = None
    ...
    def get_kwargs(self) -> _ADAMSparseStateKwargs:
    ...
    def set_new_hyper_param(self, params: Tuple[float, ...], /) -> None:
        all_p = 3 + self.layer_param_cnt(True)
        if len(params) != all_p:
        ...
        self.layer_set_new_hyper_param(params[3:])
...
_ADAMSparseStateTypes: Final[Dict[str, Tuple[Callable[[str], object], str]]] = {
...
def get_adam_sparse_state_types() -> Dict[str, Tuple[Callable[[str], object], str]]:
    return _ADAMSparseStateTypes
