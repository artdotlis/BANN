...
@final
@dataclass
class _ADAMWStateKwargs(OptimStateKwargs):
    lr: float = 0.0001
...
@final
class ADAMWState(MainOptimSt[_ADAMWStateKwargs], PerParameterAbc):

    def __init__(self) -> None:
        super().__init__()
        self.__kwargs: Optional[_ADAMWStateKwargs] = None
        self.__l_kwargs: Dict[str, _LayerCon] = {}
        self.__l_key_order: List[str] = []

    @staticmethod
    def _order_args() -> List[str]:
        return ['lr', 'betas', 'eps', 'weight_decay', 'amsgrad']
    ...
    def get_kwargs(self) -> _ADAMWStateKwargs:
...
_ADAMWStateTypes: Final[Dict[str, Tuple[Callable[[str], object], str]]] = {
    ...
    f'{MainOptimSt.get_pre_arg()}amsgrad': (
...
def get_adamw_state_types() -> Dict[str, Tuple[Callable[[str], object], str]]:
    return _ADAMWStateTypes
