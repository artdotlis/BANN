...
from typing import final, Optional, Tuple, Union, Type, Dict, Final, Callable
...
@final
@dataclass
class _GANCon(TrainGenCon):
    d_g_weight: float = 0.5


@final
class GANTState(TrainState[_GANCon]):

    def __init__(self) -> None:
        super().__init__()
        self.__kwargs: Optional[_GANCon] = None
    ...
    def set_kwargs(self, args_dict: Dict, /) -> None:
        ...
        self.__kwargs = super().parse_dict(_GANCon, args_dict)
...
_TrainStateTypes: Final[Dict[str, Tuple[Callable[[str], object], str]]] = {
    f'{TrainState.get_pre_arg()}d_g_weight': (
...
def get_train_gan_state_types() -> Dict[str, Tuple[Callable[[str], object], str]]:
    bann_gen_trainer = get_train_general_state_types()
    merged_dict = {**_TrainStateTypes}
    for key, value in bann_gen_trainer.items():
        if key in merged_dict:
            raise KnownTrainStateError(f"Duplicated key {key} in {GANTState.__name__}")
