...
def _mcc_calc(tp_c: int, tn_c: int, fp_c: int, fn_c: int, /) -> float:
    up_v = tp_c * tn_c - fp_c * fn_c
    lo_v = (tp_c + fp_c) * (tp_c + fn_c) * (tn_c + fp_c) * (tn_c + fn_c)
    try:
        res = up_v / math.sqrt(lo_v)
...
def merge_one_class_mcc(data: List[Tuple[Dict[int, RtTfF], ...]], class_num: int,
                        step_cnt: int, /) -> Tuple[str, ...]:
    rev_list, classes_list = calc_one_class_stats(class_num, step_cnt, data)
    res = [
        (str(cl_id), str(_mcc_calc(
    ...
    cross_tab = [
        ...
        "\"MCC\": [" + ','.join(re_t[1] for re_t in res) + "]"
        "}"
    ]
