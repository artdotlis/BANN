...
def _fomr_calc(fn_c: int, tn_c: int, /) -> float:
    try:
        res = fn_c / (fn_c + tn_c)
...
def merge_one_class_fomr(data: List[Tuple[Dict[int, RtTfF], ...]], class_num: int,
                         step_cnt: int, /) -> Tuple[str, ...]:
    rev_list, classes_list = calc_one_class_stats(class_num, step_cnt, data)
    res = [
        (str(cl_id), str(_fomr_calc(
    ...
    cross_tab = [
        ...
        "\"FOMR\": [" + ','.join(re_t[1] for re_t in res) + "]"
        "}"
    ]
