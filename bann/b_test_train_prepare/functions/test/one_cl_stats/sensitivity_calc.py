...
def sensitivity_calc(tp_c: int, fn_c: int, /) -> float:
    try:
        res = tp_c / (tp_c + fn_c)
...
def merge_one_class_sensitivity(data: List[Tuple[Dict[int, RtTfF], ...]], class_num: int,
                                step_cnt: int, /) -> Tuple[str, ...]:
    rev_list, classes_list = calc_one_class_stats(class_num, step_cnt, data)
    res = [
        (str(cl_id), str(sensitivity_calc(
    ...
    cross_tab = [
        "\"OneClass_Sensitivity\": {",
        "\"ClassID\": [" + ','.join(re_t[0] for re_t in res) + "],",
        "\"Sensitivity\": [" + ','.join(re_t[1] for re_t in res) + "]"
        "}"
    ]
