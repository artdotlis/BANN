...
def fnr_calc(fn_c: int, tp_c: int, /) -> float:
    try:
        res = fn_c / (tp_c + fn_c)
...
def merge_one_class_fnr(data: List[Tuple[Dict[int, RtTfF], ...]], class_num: int,
                        step_cnt: int, /) -> Tuple[str, ...]:
    rev_list, classes_list = calc_one_class_stats(class_num, step_cnt, data)
    res = [
        (str(cl_id), str(fnr_calc(
    ...
    cross_tab = [
        ...
        "\"FNR\": [" + ','.join(re_t[1] for re_t in res) + "]"
        "}"
    ]
