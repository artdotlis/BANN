...
def _fdr_calc(fp_c: int, tp_c: int, /) -> float:
    try:
        res = fp_c / (tp_c + fp_c)
...
def merge_one_class_fdr(data: List[Tuple[Dict[int, RtTfF], ...]], class_num: int,
                        step_cnt: int, /) -> Tuple[str, ...]:
    rev_list, classes_list = calc_one_class_stats(class_num, step_cnt, data)
    res = [
        (str(cl_id), str(_fdr_calc(
    ...
    cross_tab = [
        ...
        "\"FDR\": [" + ','.join(re_t[1] for re_t in res) + "]"
        "}"
    ]
