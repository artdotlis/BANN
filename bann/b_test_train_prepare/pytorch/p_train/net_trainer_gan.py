...
from typing import Final, final, Tuple, Callable, Iterable, List, Optional, Union, Dict
...
from bann.b_test_train_prepare.pytorch.p_train.functions.p_train_gan_fun import \
    PGanTrainEpochFunReturn, p_train_epoch_gan, p_gan_train_epoch_fun
from bann.b_test_train_prepare.pytorch.p_train.functions.p_train_gen_fun import PQueueTupleErg, \
    SimpleSGDOptim, PTrainEpochFun, t_print_to_logger
...
_TRAIN_LOSS_STR_GEN: Final[str] = 'Train_Loss_Gen'
_EVAL_LOSS_STR_GEN: Final[str] = 'Eval_Loss_Gen'

_EVAL_TR_V_STR: Final[str] = 'Eval_Truth'
_TRAIN_TR_V_STR: Final[str] = 'Train_Truth'
_EVAL_TR_V_STR_GEN: Final[str] = 'Eval_Truth_GEN'
_TRAIN_TR_V_STR_GEN: Final[str] = 'Train_Truth_GEN'


def add_mod_str(data_to_spl: str, /) -> str:
    if data_to_spl in [_EVAL_LOSS_STR, _EVAL_TR_V_STR, _EVAL_LOSS_STR_GEN, _EVAL_TR_V_STR_GEN]:
        return "eval"
    if data_to_spl in [_TRAIN_LOSS_STR, _TRAIN_TR_V_STR, _TRAIN_LOSS_STR_GEN, _TRAIN_TR_V_STR_GEN]:
        return "train"
    return ""
...
def _train_fun(stop_at_loss: float, args_cont: _TrainArgsCon, train_epoch: _TrfT, /) \
    ...
    epoch = 1
    running_loss_g: float = float('inf')
    running_loss_d: float = float('inf')
    report_cnt = 0
    while epoch < max_epoch + 1:
        int_max = int(epoch + report_size)
        int_max = max_epoch + 1 if int_max > max_epoch + 1 else int_max
        erg_loss: List[PGanTrainEpochFunReturn] = list(
            p_train_epoch_gan(train_loader, test_loader, epoch, int_max, train_epoch)
        )
        if erg_loss and not (
                math.isnan(erg_loss[-1].running_loss_d) or math.isnan(erg_loss[-1].running_loss_g)
        ):
            running_loss_g = erg_loss[-1].running_loss_g
            running_loss_d = erg_loss[-1].running_loss_d
        last_run = int_max >= max_epoch + 1 or (
            0 < running_loss_d < stop_at_loss
            and 0 < running_loss_g < stop_at_loss
            and report_cnt >= 2
        )
        yield (
            PQueueTupleErg(
                _TRAIN_LOSS_STR, epoch, [
                    elem.test_train.test_loss_d
                    if not (math.isnan(elem.test_train.test_loss_d)
                            or math.isinf(elem.test_train.test_loss_d))
                    else -1.0
                    for elem in erg_loss
                ], last_run
            ),
            PQueueTupleErg(
                _TRAIN_LOSS_STR_GEN, epoch, [
                    elem.test_train.test_loss_g
                    if not (math.isnan(elem.test_train.test_loss_g)
                            or math.isinf(elem.test_train.test_loss_g))
                    else -1.0
                    for elem in erg_loss
                ], last_run
            ),
            PQueueTupleErg(
                _EVAL_LOSS_STR, epoch, [
                    elem.test_eval.test_loss_d
                    if not (math.isnan(elem.test_eval.test_loss_d)
                            or math.isinf(elem.test_eval.test_loss_d))
                    else -1.0
                    for elem in erg_loss
                ], last_run
            ),
            PQueueTupleErg(
                _EVAL_LOSS_STR_GEN, epoch, [
                    elem.test_eval.test_loss_g
                    if not (math.isnan(elem.test_eval.test_loss_g)
                            or math.isinf(elem.test_eval.test_loss_g))
                    else -1.0
                    for elem in erg_loss
                ], last_run
            ),
            PQueueTupleErg(
                _TRAIN_TR_V_STR, epoch, [elem.test_train.truth_v_d for elem in erg_loss], last_run
            ),
            PQueueTupleErg(
                _TRAIN_TR_V_STR_GEN, epoch,
                [elem.test_train.truth_v_g for elem in erg_loss], last_run
            ),
            PQueueTupleErg(
                _EVAL_TR_V_STR, epoch, [elem.test_eval.truth_v_d for elem in erg_loss], last_run
            ),
            PQueueTupleErg(
                _EVAL_TR_V_STR_GEN, epoch,
                [elem.test_eval.truth_v_g for elem in erg_loss], last_run
            )
        )
        epoch = int_max
        if 0 < running_loss_d < stop_at_loss and 0 < running_loss_g < stop_at_loss \
                and report_cnt >= 2:
            epoch = max_epoch + 1
        report_cnt += 1
...
def _train_wrapper(wr_container: _TrainWrArgs, /) -> Iterable[Tuple[PQueueTupleErg, ...]]:
    ...
    def train_epoch(data_loader_train: Tuple[DataLoader, ...],
                    ...
                    epoch: int, batch_cnt: int) -> PGanTrainEpochFunReturn:
        return p_gan_train_epoch_fun(PTrainEpochFun(
...
class GANTrainer(TrainerInterface):
    ...
    def __init__(self) -> None:
        ...
        self.__train_state: Optional[GANTState] = None
        self.__log = True
    ...
    @property
    def train_state(self) -> GANTState:
        if self.__train_state is None or not isinstance(self.__train_state, GANTState):
    ...
    def set_train_state(self, state: TrainState, /) -> None:
        if not isinstance(state, GANTState):
            raise KnownTrainerError(
                f"Expected type {GANTState.__name__} got {type(state).__name__}"
            )
    ...
    @staticmethod
    def _check_truth(truth_v: float, /) -> float:
        eval_truth_local = -1.0
        if not math.isnan(truth_v) and 0 <= truth_v < float('inf'):
            eval_truth_local = truth_v
        return eval_truth_local

    def _check_loss(self, eval_loss: float, train_loss: float, /) -> float:
        ...
        return eval_loss_local

    def _set_fitness(self, eval_loss: Tuple[float, float], train_loss: Tuple[float, float],
                     eval_truth: Tuple[float, float], /) -> None:
        tr_st = self.train_state.get_kwargs()
        self.__fitness = (
            (
                tr_st.d_g_weight * self._check_loss(eval_loss[0], train_loss[0])
                + (1. - tr_st.d_g_weight) * self._check_loss(eval_loss[1], train_loss[1])
            ),
            (
                tr_st.d_g_weight * self._check_truth(eval_truth[0])
                + (1. - tr_st.d_g_weight) * self._check_truth(eval_truth[1])
            )
        )
    ...
    def train(self, sync_out: SyncStdoutInterface,
        ...
        eval_loss_truth = _EvalLossTruth()
        for tuple_erg in _train_wrapper(
            ...
            for res in tuple_erg:
                if self.train_state.get_kwargs().plot_data \
                    ...
                    if res.series not in [
                            _EVAL_LOSS_STR, _EVAL_LOSS_STR_GEN,
                            _TRAIN_LOSS_STR, _TRAIN_LOSS_STR_GEN
                    ]:
                ...
                if _EVAL_TR_V_STR == res.series:
                    eval_loss_truth.truth_d = res.y_cords[-1]
                if _EVAL_TR_V_STR_GEN == res.series:
                    eval_loss_truth.truth_g = res.y_cords[-1]
                if _EVAL_LOSS_STR == res.series:
                    eval_loss_truth.eval_d = res.y_cords[-1]
                if _EVAL_LOSS_STR_GEN == res.series:
                    eval_loss_truth.eval_g = res.y_cords[-1]
                if _TRAIN_LOSS_STR == res.series:
                    eval_loss_truth.loss_d = res.y_cords[-1]
                if _TRAIN_LOSS_STR_GEN == res.series:
                    eval_loss_truth.loss_g = res.y_cords[-1]
        self.__state_dict = deepcopy(model.state_dict())
        self._set_fitness(
            (eval_loss_truth.eval_d, eval_loss_truth.eval_g),
            (eval_loss_truth.loss_d, eval_loss_truth.loss_g),
            (eval_loss_truth.truth_d, eval_loss_truth.truth_g)
        )
