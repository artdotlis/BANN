...
from bann_demo.pytorch.networks.b_nets.simple_demo import create_pre_training_demo_net
...
@final
class SimpleDemoPreCon(SimpleNetCon[None]):
    ...
    def _create_current_net(self) -> CurrentNetData:
        return create_pre_training_demo_net(LegoContInit())
...
@final
class SimpleDemoPreNet(SimpleDemoNet):
    def _create_net(self, state_args: InitGeneralState,
        ...
        if net_load is not None:
            if net_load.is_absolute() and net_load.exists() and net_load.is_file():
                ...
                with net_load.open('rb') as net_handler:
                    ...
                    if not isinstance(current_net, SimpleDemoPreCon):
                        raise KnownSimpleDemoError(
                            f"Expected net {SimpleDemoPreCon.__name__} "
        ...
        if self.net_module is None:
            current_net = SimpleDemoPreCon()
            self.net_module_set(current_net)
            self.random_net_set(net_state.get_kwargs().random)

            self.savable_set(NetSavable[nn.Module, CurrentNetData, InitContainer](
                current_net,
                NetSavableArgs(
                    node_id=ENetInterfaceNames.SIMPLEDEMOPRENET.value,
                    node_type=type(self),
